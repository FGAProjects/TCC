...
\subsection{Integração Contínua e Deploy}

Para a integração contínua, explicada no tópico \ref{sec:integracao_continua}, foi escolhida a ferramenta \textit{TravisCI}. Essa ferramenta possui integração com o \textit{GitHub}, que é a ferramenta para o versionamento do código gerado. Ao realizar um \textit{commit} pelo \textit{GitHub}, o \textit{TravisCI} irá analisar o código e verificar se algo está comprometendo o ambiente de desenvolvimento. Toda vez que o \textit{TravisCI} consegue gerar ou não uma \textit{build}, é enviado para o \textit{e-mail} do responsável um informe sobre o status do projeto.
