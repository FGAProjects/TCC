...
Segundo \citeonline{integracao_continua}, integração contínua é um processo são realizados em ambientes de desenvolvimento, em que há uma solicitação de uma manutenção e a implantação da mesma em produção. Esse processo gera ciclos mais longos a cada nova versão do \textit{software}.

Para \citeonline{integracao_continua}, a integração contínua se resume em práticas que buscam a redução dos ciclos de integração dos componentes de \textit{software}, atráves de ferramentas que automatizam o versionamento do código, a construção e demais processos de desenvolvimento.

Os principais objetivos da integração contínua são: redução de custos e tempo de integração entre componentes de \textit{software}, redução dos riscos, aumento da qualidade e foco na automação dos processos. 
